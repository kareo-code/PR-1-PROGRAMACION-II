...
bool DEBUG = true; //para que se imprima el debug del sexo y fuerza debe de estar este activado
bool DEBUG_SEXO = true;
bool DEBUG_FUERZA = true;
        ...
        for (int i = 0; i < SIZE; i++) {
            for (int j = 0; j < SIZE; j++) {
                if (grid[i][j] != NULL) {
                    ...
                    if (DEBUG_SEXO) {
                        cout << " Sexo:" << (grid[i][j]->sexo ? "M" : "H");
                    }

                    if (DEBUG_FUERZA) {
                        cout << " Fuerza:" << grid[i][j]->fuerza;
                    }
